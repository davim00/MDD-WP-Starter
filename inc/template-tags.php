...
function wp_starter_posted_on() {
	...
	$posted_on = sprintf(
		...
		esc_html_x( 'Posted on %s', 'post date', 'wp-starter' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		/* translators: %s: post author. */
		esc_html_x( 'by %s', 'post author', 'wp-starter' ),
...
function wp_starter_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'wp-starter' ) );
		if ( $categories_list && wp_starter_categorized_blog() ) {
			/* translators: 1: list of categories. */
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'wp-starter' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'wp-starter' ) );
		if ( $tags_list ) {
			/* translators: 1: list of tags. */
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'wp-starter' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
		...
		comments_popup_link(
			sprintf(
				wp_kses(
					...
					__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'wp-starter' ),
					array(
						'span' => array(
							'class' => array(),
						),
	...
	edit_post_link(
		sprintf(
			wp_kses(
				...
				__( 'Edit <span class="screen-reader-text">%s</span>', 'wp-starter' ),
				array(
					'span' => array(
						'class' => array(),
					),
...
function wp_starter_categorized_blog() {
	$all_the_cool_cats = get_transient( 'wp_starter_categories' );
